...
# Shared arrays for fast access in workers via global
# We'll use mp.Pool with initializer to avoid pickling large arrays per task
X_train_g=X_train
y_train_g=y_train
X_test_g=X_test
X_submit_g=X_submit
...
# init with global for resume safety
...
for start in range(n_done, total, BATCH):
    ...
    with mp.Pool(N_JOBS, initializer=init_worker, initargs=(X_train_g,y_train_g,X_test_g,X_submit_g,pred_global_test_g,pred_global_submit_g,user_to_train_idx_g,uid_to_test_idx_g,uid_to_submit_idx_g,tmp_gm_g)) as pool:
        results=pool.map(worker, batch_uids)
    # results ordered as batch_uids
...
# Save submit to Desktop and reports
preds_submit_y=np.maximum(np.expm1(preds_submit),0.0)
# Need order by user_id sorted, our preds_submit_all is already sorted by all_uids_sorted
...
# Also save to submissions for repo (gitignored)
