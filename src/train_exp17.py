...
EXP17_DIR = Path("data/v2/features_exp17")
REPORT_PATH = Path("reports/stack_search_exp17.json")
...
def load_fold(name: str) -> pl.DataFrame:
    ...
    exp17_feats = pl.read_parquet(EXP17_DIR / name / "batch_*.parquet")
    df = feats.join(btyd, on=["anchor_date", "user_id"], how="left")
    df = df.join(exp17_feats, on=["anchor_date", "user_id"], how="left")
...
def main() -> None:
    ...
    subs = {
        "stack_exp17": save(stack_val, "submission_stack_exp17.csv"),
        "stack_exp17_calib": save(calib_val, "submission_stack_exp17_calib.csv"),
    }
