"""Стек v3 на очищенном датасете data/v3 (85 фичей после голосования).

Цель: сравнить с референсами на том же протоколе —
exp01 CatBoost solo 1.70261 | стек v2 1.67103 (fold_03 holdout).

Протокол как в train_stack_v2.py:
  A  тюнинг компактных гридов: train fold_00+01 -> ES/val fold_02
  B  OOF leave-one-fold-out по fold_00..02
  B2 прод-бейзы на fold_00..02 (ES fold_02) -> предикты fold_03 и fold_end
  C  мета (ridgecv/ols_pos/lgbm_meta) выбирается внутри OOF (fit 00+01, val 02)
  D  калибровка c x tau на inner-OOF
  E  скоры fold_03 + сабмиты

Run from repo root:
    .venv/bin/python src/train_stack_v3.py [--quick]
Artifacts:
    reports/stack_search_v3.json, submissions/submission_{v3_cat,v3_lgbm,stack_v3,stack_v3_calib}.csv
"""
...
V3_DIR = Path("data/v3")
REPORT_PATH = Path("reports/stack_search_v3.json")
...
def main() -> None:
    ...
    for name, grid in grids.items():
        ...
        for i, params in enumerate(grid):
            preds, model = fit_predict(name, params, tune_tr_X, tune_tr_y,
            ...
            if name == "cat" and s == min(r[0] for r in rows):
                cat_importances = sorted(
                    zip(feats, model.get_feature_importance()),
                    key=lambda t: -t[1])
    ...
    print("LEADERBOARD fold_03 (exp01 1.70261 | stack v2 1.67103):")
    board = sorted({**solo,
                    "STACK_v3_" + best_meta_name: stack_raw_score,
                    "STACK_v3_calib": stack_calib_score}.items(),
                   key=lambda kv: kv[1])
    for n, s in board:
    ...
    subs = {
        "v3_cat": save(Z_end_m["cat"], "submission_v3_cat.csv"),
        "v3_lgbm": save(Z_end_m["lgbm"], "submission_v3_lgbm.csv"),
        "stack_v3": save(stack_val, "submission_stack_v3.csv"),
        "stack_v3_calib": save(calib_val, "submission_stack_v3_calib.csv"),
    }
    ...
    REPORT_PATH.write_text(json.dumps({
        ...
        "dataset": "data/v3 (85 features, pruned schema)",
        "features": len(feats),
        "members": MEMBERS,
        "grids": grids,
        "best_params": all_params,
        "cat_importance_top20": [(f, round(float(v), 2)) for f, v in (cat_importances or [])[:20]],
